...
function pillCell(status) {
  // Muted Light palette — print-safe; all five checkers SAFE/PASS.
  const colours = {
    pass:    { backgroundColor: '#DDEEE2', color: '#1E5631' },
    fail:    { backgroundColor: '#F0DCDC', color: '#8A2A2A' },
    warn:    { backgroundColor: '#EDE6D2', color: '#4E3C18' },
    neutral: { backgroundColor: '#E7EAED', color: '#4A535D' },
  };
...
async function buildDocument(entries, timestamp) {
  ...
  artifact(doc, (d) => {
    d.rect(doc.page.margins.left, headerTop, PAGE_W, HEADER_H).fill('#234A73'); /* Muted Light --accent */
  });
  ...
  h1.add(() => {
    ...
    doc.font('Medium').fontSize(18)
       .fillColor('#FFFFFF') /* --accent-text on #234A73 header */
       .text('Image Colour ', x, doc.y, { continued: true, link: null, underline: false, oblique: false, width: HEADER_TEXT_W })
       .fillColor('#FFFFFF') /* keep white: --warm (#703D1F) does not contrast on #234A73 */
  ...
  taglineStruct.add(() => {
    const x = doc.page.margins.left + HEADER_PAD_L;
    doc.font('Regular').fontSize(8.75).fillColor('#FFFFFF') /* white on #234A73 header */
       .text(
  ...
  doc.fillColor('#2B3138'); /* Muted Light --fg */
  addHeading(doc, 2, 'Audit Report',            { font: 'Medium', fontSize: 14 });
  addParagraph(doc, `Generated ${timestamp}`,   { fontSize: 9, fillColor: '#5A636D' });
  writeParagraph(doc, [
    { text: 'Generated by ',  fontSize: 9, color: '#5A636D' },
    { text: APP_NAME,         fontSize: 9, color: '#234A73', link: SITE_URL }, /* Muted Light --accent */
  ]);
  doc.moveDown(0.5);
  ...
  artifact(doc, (d) => {
    d.roundedRect(doc.page.margins.left, disclTop, PAGE_W, DISCL_H, 4).fill('#EDE6D2'); /* Muted Light --warn-bg */
  });
  ...
  disclStruct.add(() => {
    ...
    doc.font('Medium').fontSize(9).fillColor('#2B3138') /* Muted Light --fg */
  ...
  doc.table({
    ...
    data: [
      [
        { type: 'TH', scope: 'column', text: 'Image',  font: { src: 'Medium' }, backgroundColor: '#E7EAED' }, /* Muted Light --neutral-bg */
        { type: 'TH', scope: 'column', text: 'Result', font: { src: 'Medium' }, backgroundColor: '#E7EAED' },
      ],
      ...entries.map((e) => {
  ...
  entries.forEach((entry, idx) => {
    ...
    const vsColor = { pass: '#1E5631', fail: '#8A2A2A', warn: '#4E3C18' }[statusStyle(vs)] || '#5A636D'; /* Muted Light palette */
    doc.fillColor('#2B3138');
    writeParagraph(doc, [
      { text: 'Result: ',              font: 'Medium',  fontSize: 10, color: '#2B3138' },
      { text: vs,                      font: 'Medium',  fontSize: 8,  color: vsColor },
      { text: ` — ${entry.report.detail}`, font: 'Regular', fontSize: 10, color: '#2B3138' },
    ]);
    ...
    if (entry.cbSimAssets && entry.cbSimAssets.length) {
      ...
      for (let row = 0; row < Math.ceil(entry.cbSimAssets.length / CVD_COLS); row++) {
        ...
        for (let col = 0; col < CVD_COLS; col++) {
          ...
          lblStruct.add(() => {
            doc.font('Regular').fontSize(8).fillColor('#5A636D') /* Muted Light --fg-muted */
    ...
    if (entry.report.hasText && entry.report.colourPairs && entry.report.colourPairs.length) {
      ...
      doc.moveDown(0.3);
      addParagraph(doc, wcagLine(entry.report),     { fontSize: 10, fillColor: '#5A636D', oblique: true }); /* Muted Light --fg-muted */
      addParagraph(doc, advancedLine(entry.report), { fontSize: 10, fillColor: '#5A636D', oblique: true });
      ...
      for (const pair of entry.report.colourPairs) {
        ...
        addParagraph(doc,
          ...
          { font: 'Regular', fontSize: 10, fillColor: '#2B3138' }
        );
        ...
        const exampleSpans = pair.examples && pair.examples.length
          ? [{ text: pair.examples.map((e) => `"${e}"`).join(', ') + '   ', fontSize: 9, color: '#5A636D', oblique: true }]
          : [];
        writeParagraph(doc, [
          ...exampleSpans,
          { text: `Check ${pair.fgHex}/${pair.bgHex} on WebAIM Contrast Checker`, fontSize: 9, color: '#234A73', link: webaim }, /* Muted Light --accent */
          { text: '   ',                             fontSize: 9, color: '#5A636D' },
          { text: VESTIBULAR_CHECKER_FULL_LABEL,     fontSize: 9, color: '#234A73', link: VESTIBULAR_CHECKER_URL },
        ]);

        // 1.9.3 Checks table — Check column cells carry hyperlinks (requires addTable)
        const checks     = pairChecks(pair);
        const tableData  = [[
          { type: 'TH', scope: 'column', content: 'Check',         font: 'Medium', fontSize: 9, backgroundColor: '#E7EAED' }, /* Muted Light --neutral-bg */
          { type: 'TH', scope: 'column', content: 'Value',         font: 'Medium', fontSize: 9, backgroundColor: '#E7EAED' },
          { type: 'TH', scope: 'column', content: 'Status',        font: 'Medium', fontSize: 9, backgroundColor: '#E7EAED' },
          { type: 'TH', scope: 'column', content: 'What it means', font: 'Medium', fontSize: 9, backgroundColor: '#E7EAED' },
        ]];
        ...
        for (const grp of CHECK_GROUPS) {
          tableData.push([
            { type: 'TH', scope: 'row', content: grp.label,
              ...
              backgroundColor: '#D2D8DE', color: '#2B3138' }, /* Muted Light --border / --fg */
            { content: '' }, { content: '' }, { content: '' },
          ]);
          for (const c of checks.filter((ch) => ch.group === grp.id)) {
            const pill = pillCell(c.status);
            tableData.push([
              { content: [{ text: c.label, link: checkInfoUrl(c.id), color: '#234A73', underline: true }] }, /* Muted Light --accent */
              { content: c.value || '—' },
        ...
        if (asset?.clipDataUrl) {
          addParagraph(doc, 'Where this combination appears:', {
            fontSize: 9, fillColor: '#5A636D', oblique: true, /* Muted Light --fg-muted */
          });
  ...
  doc.moveDown();
  doc.fillColor('#2B3138');
  addParagraph(doc, THRESHOLDS_FOOTER, { fontSize: 9, fillColor: '#5A636D', oblique: true }); /* Muted Light --fg-muted */
  writeParagraph(doc, [
    { text: 'Generated by ', fontSize: 9, color: '#5A636D' },
    { text: APP_NAME,        fontSize: 9, color: '#234A73', link: SITE_URL }, /* Muted Light --accent */
  ]);
